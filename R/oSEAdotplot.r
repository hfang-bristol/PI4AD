...
#'
#' @param obj an object of class "eSET" or "eSAD". Alterntively, it can be a data frame having all these columns (named as 'name','adjp','zscore'), optionally having these columns ('nO','group','onto','namespace')
...
oSEAdotplot <- function(obj, FDR.cutoff=0.05, colors=c("pink","red"), y.scale=c("normal","log"), slim=NULL, size.range=c(0.5,3.5), size.title="Num of overlaps", label.top='auto', label.direction.y=c("none","left","right"), label.size=2, ...)
{
    ...
    group <- onto <- name <- nO <- adjp <- zscore <- namespace <- NULL
	...
	if(is(df,'data.frame')){
		...
		if(all(c('name','adjp','zscore') %in% colnames(df))){
